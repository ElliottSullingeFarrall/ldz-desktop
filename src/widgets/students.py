...
class StudentsRegular(Widget):
    def __init__(self, profile: Profile) -> None:
        super().__init__(
            profile, labels=("# Students", "Project"), defaults=("1", "No")
        )
        ...
        self.field0 = ttk.Spinbox(
            ...
            values=list(map(str, range(1, 1000))),
            state="readonly",
            width=10,
        )
        self.label1 = ttk.Label(self.profile, text=f"{self.labels[1]}:")
        self.field1 = ttk.Checkbutton(
            self.profile, variable=self.vars[1], offvalue="No", onvalue="Yes"
        )
        ...
        self.field1.grid(row=self.row, column=3, sticky="w")


class StudentsEmbedded(Widget):
    def __init__(self, profile: Profile) -> None:
        super().__init__(profile, labels=("Expected", "Arrived"), defaults=("1", "1"))
        self.values = (list(map(str, range(1, 1000))), list(map(str, range(1, 1000))))
        ...
        self.labelf = ttk.Label(self.profile, text="# Students ")
