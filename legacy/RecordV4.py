...
def load_config(filename):
    if exists(filename):
        try:
            config = load_workbook(filename=filename)
...
config = load_config(filename='config.xlsx')
departments = [cell.value for cell in config.active['A']]
queries     = [cell.value for cell in config.active['B']]
levels      = [cell.value for cell in config.active['C']]
types       = [cell.value for cell in config.active['D']]
locations   = [cell.value for cell in config.active['E']]
config.save(filename='config.xlsx')
config.close()

window = Tk()
window.resizable(False, False)
window.title('Record')
window.iconbitmap('stag.ico')
...
def submit_func():
    if '' in get_vars().values():
        ...
        return
    data = load_data(filename='data.xlsx')
    data.active.append(list(get_vars().values()))
    data.save(filename='data.xlsx')
    data.close()
    init_vars()        
...
def del_func():
    ...
    del_window.iconbitmap('stag.ico')
    del_window.attributes('-topmost', 'true')
    del_window.grab_set()

    data = load_data(filename='data.xlsx')
    table = ttk.Treeview(del_window, columns=[cell.value for cell in data.active[1]], show='headings', selectmode='browse')
